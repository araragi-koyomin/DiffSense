...
## Phase 2: 多仓库 SaaS Web 模式 — Brainstorming 记录

### 触发背景

Phase 1 完成后，用户尝试部署到华为云 ECS。发现当前架构只能预先绑定一个仓库路径，新用户无法通过浏览器提交自己的仓库。用户明确表达了 SaaS 需求：

> "不能做到：我是一个新用户，我进入这个网站，配置好我的 api，然后上传一个 github 仓库链接上去，然后开始分析是吗？"

### 关键决策节点

| 轮次 | 决策点 | 选项 | 用户选择 | 影响 |
|------|--------|------|---------|------|
| Q1 | 用户模型 | 无账号 / 轻量用户 / 完整账号 | **A: 无账号** | 免去注册登录系统，简化架构 |
| Q2 | 仓库生命周期 | 会话级 / 持久化 / 手动管理 | **A: 会话级 30min** | 需要 session TTL + 定时清理 |
| Q3 | API Key 处理 | 浏览器存储 / 服务端加密 / 环境变量 | **B: 服务端加密** | 需要 AES-256-GCM 加密模块，且不能依赖全局配置的 key |
| Q4 | 首页设计 | 单页 / 仓库列表 / 演示模式 | **A: 单页** | 最简实现，一个输入框 |
| Q5 | 数据隔离 | 独立 DB / 共享 DB | **A: 独立 DB** | 复用现有 .diffsense.db 逻辑 |
| Q6 | 会话机制 | Cookie / URL 参数 / 不恢复 | **A: Cookie** | 需要 cookie-parser 中间件 |
| Q7 | Clone 失败 | 即时提示 / 支持私有 | **A: 即时提示** | 简化错误处理 |
| Q8 | 添加后行为 | 自动分析 / 手动选择 | **B: 手动选择** | 复用现有列表页，改动量最小 |

### AI 建议采纳与推翻

| 建议 | 来源 | 采纳/推翻 | 理由 |
|------|------|-----------|------|
| 方案 B（轻量用户，API Key 作为身份） | AI | 推翻 | 用户不想记账号，要零门槛 |
| 方案 B（持久化仓库） | AI | 推翻 | 磁盘无限增长风险，用户选择简单 |
| 方案 A（仅存浏览器 Key） | AI | 推翻 | 用户希望服务端存储，避免重复输入 |

### 设计反思

- **用户对 SaaS 的直觉是"打开网页就能用"。** 所有本地化操作（clone、配置、环境变量）都应隐藏到服务端。
- **会话级生命周期是最简的隔离方案。** 不需要持久化存储、不需要用户管理界面，30 分钟后自动清理且无残留。
- **服务端加密存储 API Key 引入了额外的安全责任**（密钥管理、加密算法选择），但这是用户明确的要求。

### 本次 brainstorming 的评价

**好的地方：**
- 8 个问题覆盖了 SaaS 改造的全部关键决策点
- 选项设计简洁，每轮 2-3 个，用户决策无困惑
- 最终方案与现有架构兼容度最高（独立 DB、复用列表页、最小侵入）

**不足：**
- 未讨论并发 clone 场景（多个用户同时添加仓库时的磁盘 IO 压力）
- 未讨论 `.apikey` 文件的权限安全（0700 权限设置）
- 未讨论 session 内存泄露风险（定时清理的边界条件）
