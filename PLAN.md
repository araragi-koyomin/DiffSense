...
## Phase 15: 多仓库 SaaS Web 模式

> **致智能体工作者:** 必需的子技能：使用 superpowers:subagent-driven-development（推荐）来逐任务实现本计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**目标:** 将 DiffSense 改造为 SaaS 形态——访客输入 GitHub URL + API Key 即可分析任意公开仓库，无需本地部署。

**架构:** 新增 session 管理、API Key 加密存储、git clone 服务端模块。Web 路由从单一 repo 模式改为 session 驱动。首页新增 landing page。

**技术栈:** 与现有一致，新增 `crypto`（Node.js 内置）用于 API Key 加密。
...
T30（session 管理）
  └─ T31（API Key 加密）
       └─ T32（repo cloner）
            └─ T33（landing page + 路由改造）
                 └─ T34（现有路由适配 session）
                      └─ T35（docker-compose + README）
```
...
**涉及文件:**
- 新增: `src/web/session.ts`

**功能:** 内存 session 存储，支持创建/查询/刷新 TTL/自动清理
...
export interface Session {
  token: string;
  repoPath: string;   // 如 repos/<token>/owner-repo/
  createdAt: number;
  lastAccess: number;
}
...
```bash
git add src/web/session.ts
git commit -m "feat: session 管理器（内存存储 + TTL 过期 + 定时清理）（T30）"
```
...
**涉及文件:**
- 新增: `src/web/crypto-keys.ts`

- [ ] **第1步：编写 crypto-keys.ts**

```typescript
// src/web/crypto-keys.ts
...
```bash
git add src/web/crypto-keys.ts
git commit -m "feat: API Key AES-256-GCM 加密/解密模块（T31）"
```

---

### T32: 服务端 Git Clone 模块

**涉及文件:**
- 新增: `src/web/cloner.ts`
...
```bash
git add src/web/cloner.ts
git commit -m "feat: 服务端 git clone 模块（超时 30s, depth 50）（T32）"
```

---

### T33: Landing Page + Session 路由

**涉及文件:**
- 新增: `src/web/views/landing.html`
- 修改: `src/web/index.ts`（添加 cookie-parser，注入 session middleware）
- 修改: `src/web/routes/pages.ts`（GET / 按 session 分流，新增 POST /add-repo）

**依赖:** `npm install cookie-parser @types/cookie-parser`

- [ ] **第1步：安装 cookie-parser**
...
```html
<!-- src/web/views/landing.html -->
<div style="max-width:480px;margin:4rem auto;">
  <h2 style="margin-bottom:0.5rem;">DiffSense</h2>
  <p style="color:var(--accents-5);margin-bottom:2rem;">输入 GitHub 公开仓库链接，AI 自动分析 commit 语义</p>

  <form method="POST" action="/add-repo" style="display:flex;flex-direction:column;gap:1rem;">
    <div>
      <label style="font-size:0.85rem;color:var(--accents-5);display:block;margin-bottom:0.3rem;">GitHub 仓库 URL</label>
      <input type="text" name="url" placeholder="https://github.com/owner/repo"
        style="width:100%;padding:0.6rem 0.8rem;border:1px solid var(--accents-2);border-radius:var(--geist-radius);font-size:0.9rem;"
        required />
    </div>
    <div>
      <label style="font-size:0.85rem;color:var(--accents-5);display:block;margin-bottom:0.3rem;">API Key（DeepSeek 或 GLM）</label>
      <input type="password" name="apiKey" placeholder="sk-..."
        style="width:100%;padding:0.6rem 0.8rem;border:1px solid var(--accents-2);border-radius:var(--geist-radius);font-size:0.9rem;"
        required />
    </div>
    {{{error}}}
    <button type="submit" class="btn" style="padding:0.7rem;">开始分析</button>
  </form>
</div>
```

- [ ] **第3步：修改 index.ts — 注入 session middleware + cookie-parser**
...
- [ ] **第4步：修改 pages.ts — GET / 分流 + 新增 POST /add-repo**

在 `src/web/routes/pages.ts` 顶部新增 import：
...
在 `registerPageRoutes` 函数顶部（`const rp = ...; const dbPath = ...;` 之前）添加 session 解析逻辑：

```typescript
  // Session middleware: 包装 app.get/post
  const sessionRepo = (req: Request): string | null => {
...
修改 `app.get('/')` 的首行，在路由开始处增加 landing 分流：

```typescript
  app.get('/', async (req: Request, res: Response) => {
    const sr = sessionRepo(req);
    if (!sr) {
      // 无 session → 显示 landing page
      res.send(render('landing', { error: '' }));
      return;
    }
    // 有 session → 复用现有列表页逻辑，但使用 session 的 repo 路径
    const rp = sr;
...
注意：这意味着需要把 `rp` 和 `dbPath` 从函数级变量改为路由内动态获取。为了减少对现有路由的侵入，更简单的方式是：只修改 `GET /` 路由内的前几行，后续逻辑不变。具体做法是把函数顶部的 `const rp = ...; const dbPath = ...;` 移到每个路由内部（或使用 sessionRepo 优先）。

**简化方案：** 在 `GET /` 路由内，优先用 session repo，否则用启动参数：
...
```typescript
  ...
  app.get('/', async (req: Request, res: Response) => {
    ...
    const rp = sr;
    const dbPath = path.join(rp, '.diffsense.db');
    // ... 后续逻辑与现有 GET / 完全一致（复用已写好的代码）
  });
```

需要新增 `renderLanding` 函数：
...
新增 `POST /add-repo` 路由（在 `registerPageRoutes` 函数内）：
...
```bash
git add src/web/views/landing.html src/web/index.ts src/web/routes/pages.ts package.json package-lock.json
git commit -m "feat: landing page + POST /add-repo + session cookie（T33）"
```
...
**涉及文件:**
- 修改: `src/web/routes/pages.ts`（其余路由使用 session repo）
- 修改: `src/web/routes/api.ts`（API 路由使用 session repo + session API Key）

- [ ] **第1步：pages.ts 其余路由适配**

将 `GET /commits/:hash` 和 `GET /stats` 路由中的 `rp` 和 `dbPath` 改为动态获取：

在每个路由开头增加：
```typescript
    const sr = sessionRepo(req);
    if (!sr) { res.redirect('/'); return; }
    const rp = sr;
    const dbPath = path.join(rp, '.diffsense.db');
```

替代原有的函数级变量。删除 `registerPageRoutes` 顶部的 `const rp = repoPath || process.cwd();` 和 `const dbPath = path.join(rp, '.diffsense.db');`，改为在每个路由内动态获取。

- [ ] **第2步：api.ts 适配**

`src/web/routes/api.ts` 需要同样的改动。顶部新增 import：
...
每个 API 路由内动态获取 repo 路径：

```typescript
    const token = req.cookies?.['ds_session'];
    if (!token) { res.status(401).json({ error: '请先添加仓库' }); return; }
    const s = getSession(token);
    if (!s) { res.status(401).json({ error: '会话已过期，请重新添加仓库' }); return; }
    const rp = s.repoPath;
    const dbPath = path.join(rp, '.diffsense.db');
```

对于 `POST /api/analyze` 和 `POST /api/analyze-all`，还需要从 session 获取 API Key：

```typescript
    // 读取加密的 API Key
    const keyFile = path.join(rp, '.apikey');
    let apiKey = '';
    if (fs.existsSync(keyFile)) {
      apiKey = cryptoKeys.decryptApiKey(fs.readFileSync(keyFile, 'utf-8'));
    }
    // 用 session 的 apiKey 替代 loadConfig + getApiKey
```

但 `processCommit()` 使用的是全局 config，这里需要适配。简单方案：在 `processCommit` 调用时直接传入 apiKey 而非从 config 读取。查看 `processCommit` 签名：

`processCommit(rp, hash, cfg, apiKey, dbPath, lp)` — 已经接受 apiKey 参数。所以只需传入解密的 apiKey 即可。cfg 可以从 `loadConfig()` 读取（仅需 base_url 和 model）。
...
```bash
git add src/web/routes/pages.ts src/web/routes/api.ts
git commit -m "feat: 现有路由适配 session repo 路径 + API Key（T34）"
```

---

### T35: docker-compose + README 更新

**涉及文件:**
- 修改: `docker-compose.yml`
- 修改: `README.md`

- [ ] **第1步：docker-compose.yml 添加 DIFFSENSE_SECRET**
...
services:
  diffsense:
    ...
    environment:
      - DEEPSEEK_API_KEY=${DEEPSEEK_API_KEY}
      - DIFFSENSE_SECRET=${DIFFSENSE_SECRET:-}
    command: web
```

注意：`command: web` 不再需要 `-r /repo`（因为 repo 由 session 动态管理）。
...
# 2. 设置加密密钥（可选，不设置则每次重启后旧 API Key 失效）
#    PowerShell:
$env:DIFFSENSE_SECRET = "your-random-secret-at-least-32-chars"
#    Linux/macOS:
export DIFFSENSE_SECRET="your-random-secret-at-least-32-chars"

# 3. 启动
docker-compose up
# 访问 http://localhost:9090，输入 GitHub 仓库链接 + API Key 即可开始分析
```
...
部署后，任意用户打开网站即可：
1. 输入 GitHub 公开仓库 URL（如 `https://github.com/facebook/react`）
2. 输入自己的 LLM API Key（DeepSeek 或 智谱 GLM）
3. 进入 commit 列表页，勾选需要分析的 commit
4. 浏览分析结果

每个会话独立隔离，30 分钟无操作自动清理。
```
...
git commit -m "docs: 更新 compose + README 适配 SaaS 多仓库模式（T35）"
```
...
- [ ] SPEC 覆盖率：8 项设计决策有对应 Task
- [ ] 占位符扫描：零 TBD / TODO
- [ ] 依赖标注：T30→T31→T32→T33→T34→T35（全串行）
- [ ] 每个 Task 均有验证步骤
